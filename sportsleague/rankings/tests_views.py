...
class RankingpageTests(SimpleTestCase):
    def test_url_exists_at_correct_location(self):
        response = self.client.get("/ranking/Test%20Team,%204,%20TeamTest,%203%")
        self.assertEqual(response.status_code, 200)

    def test_url_available_by_name(self):  
        response = self.client.get(reverse("ranking", kwargs={"data": "test data:2"}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_template_content(self):
        response = self.client.get(reverse("ranking", kwargs={"data": "test data:2"}))
        self.assertContains(response, "<h1>Ranking based on uploaded and edited CSV</h1>")
        self.assertNotContains(response, "Not on the page")
